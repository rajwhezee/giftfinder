...
| **eBay `/item/{id}`** | The only endpoint carrying `brand`, and it answers 403 "Insufficient permissions" to this keyset's client-credentials token. Brands come from the title instead; see the brand note above. |
